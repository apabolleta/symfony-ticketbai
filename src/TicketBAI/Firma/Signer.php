...
namespace APM\TicketBAIBundle\TicketBAI\Firma;
...
final class Signer {
    ...
    const SIGN_POLICY_NAME = 'TicketBAI sinadura zehaztapenak v1.0';
    const SIGN_POLICY_URL = 'https://www.batuz.eus/fitxategiak/batuz/ticketbai/sinadura_elektronikoaren_zehaztapenak_especificaciones_de_la_firma_electronica_v1_0.pdf';

    // TODO Hau ez dakit zer ostia dan,
    ...
    public function sign($xml) {
        ...
        $openTagPosition = mb_strpos($xml, '<T:TicketBai ');
        if ($openTagPosition === false) {
            throw new RuntimeException('XML document is missing <T:TicketBai /> element');
        }
        $closeTagPosition = mb_strpos($xml, '</T:TicketBai>');
        if ($closeTagPosition === false) {
            throw new RuntimeException('XML document is missing </T:TicketBai> closing tag');
        }
        // TODO Hemengo 14 ni puta idea zer dan
        $closeTagPosition += 14;
        ...
        // Build new document
        $xmlRoot = str_replace('</T:TicketBai>', "$dsSignature</T:TicketBai>", $xmlRoot);
